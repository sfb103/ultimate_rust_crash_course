...
fn main() {
    ...
    // let handle1 = thread::spawn( ||receive_msg(1,rx) );
    // let handle2 = thread::spawn( ||receive_msg(2,rx2) );

    let receive_msg = |id,rx|{
        for msg in rx {
            println!("thread {}: Received {}", id, msg);
            pause_ms(100);
        }};

    let handle1 = thread::spawn( move||receive_msg(1,rx) );
    let handle2 = thread::spawn( move||receive_msg(2,rx2) );

    let msgs = vec!["hello", "there", "nice", "to", "meet", "you"];
    msgs.iter().for_each(|&msg| tx.send(msg).unwrap());
    drop(tx);
...
// fn receive_msg( thread_id: i32, rx: crossbeam::Receiver<&str> ) {
//     for msg in rx {
//         println!("thread_id: {}, received msg: {}", thread_id, msg);
//         pause_ms(100);
//     }
// }
